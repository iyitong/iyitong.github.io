<!--param start-->
    //修改cdn的绝对路径（测试环境）
    fis.config.set('cdn-path','http://192.168.0.123:8080');
    //修改cdn的绝对路径（正式环境）
    fis.config.set('cdn-path-release','http://iyitong.github.io/amfq');
    //修改雪碧图放大缩小倍数，默认是1，iphone是0.5
    fis.config.set('css-scale',1);
    //修改include文件的域名
    fis.config.set('include-host','http://192.168.0.123:8080')
    //修改打包配置
    fis.config.set('pack', {
        'pkg/lib.js': [
            'src/js/lib/**.js'
